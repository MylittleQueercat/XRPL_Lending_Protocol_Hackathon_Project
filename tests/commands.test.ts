...
it('vanilla runs the lending flow when V1.1 is enabled', async () => {
  ...
  expect(await executeCommand('vanilla', { inspect: async () => report, smoke: vi.fn(), vanilla, settlement: vi.fn(), output: () => {} })).toBe(0);
...
it('vanilla refuses to fund or sign when a required amendment is missing', async () => {
  ...
  const vanilla = vi.fn();
  await expect(executeCommand('vanilla', { inspect: async () => blocked, smoke: vi.fn(), vanilla, settlement: vi.fn(), output: () => {} })).rejects.toThrow(/LendingProtocol/);
  expect(vanilla).not.toHaveBeenCalled();
});
it('doctor reports a compatible lending track when V1.1 is enabled', async () => {
  const output = vi.fn();
  expect(await executeCommand('doctor', { inspect: async () => report, smoke: vi.fn(), vanilla: vi.fn(), settlement: vi.fn(), output })).toBe(0);
...
it('vault smoke is an explicit command and remains available with V1.1', async () => {
  ...
  expect(await executeCommand('vault-smoke', { inspect: async () => report, smoke, vanilla: vi.fn(), settlement: vi.fn(), output: () => {} })).toBe(0);
  expect(smoke).toHaveBeenCalledOnce();
});
it('unknown commands do not connect or fund wallets', async () => {
  const inspect = vi.fn();
  await expect(executeCommand('typo', { inspect, smoke: vi.fn(), vanilla: vi.fn(), settlement: vi.fn(), output: () => {} })).rejects.toThrow(/Usage/);
