...
export function NetworkBadge() {
  ...
  if (!network && !networkError) return <Badge variant="outline" className="gap-1.5 font-mono"><Activity className="animate-pulse" /> connecting…</Badge>;
  if (networkError || !network?.matches) {
    return (
      <Badge variant="destructive" className="gap-1.5" title={networkError ?? undefined}>
        ...
        {network ? `wrong network ${network.networkId}` : "offline"}
      </Badge>
    );
  }
  return (
    <Badge variant="success" className="gap-1.5 font-mono" title={`${TRACK1.wsUrl} · rippled ${network.build} · ledger ${network.ledgerIndex} (${network.ledgerAgeSeconds}s)`}>
      ...
      network {network.networkId} · #{network.ledgerIndex.toLocaleString("en-US")}
    </Badge>
  );
}
